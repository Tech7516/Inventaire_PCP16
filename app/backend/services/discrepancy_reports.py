...
class Discrepancy_reportsService:
    ...
    async def get_list(
        ...
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Discrepancy_reports, field):
                        column = getattr(Discrepancy_reports, field)
            ...
            stmt = select(Discrepancy_reports, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(Discrepancy_reports, field_name):
                        stmt = stmt.order_by(getattr(Discrepancy_reports, field_name).desc())
                else:
                    if hasattr(Discrepancy_reports, sort):
                        stmt = stmt.order_by(getattr(Discrepancy_reports, sort))
            else:
                stmt = stmt.order_by(Discrepancy_reports.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Discrepancy_reports)
                for condition in conditions:
    ...
    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Discrepancy_reports]:
        ...
        try:
            ...
            column = getattr(Discrepancy_reports, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Discrepancy_reports).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
