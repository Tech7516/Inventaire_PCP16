...
class Sub_entity_checksService:
    ...
    async def get_list(
        ...
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Sub_entity_checks, field):
                        column = getattr(Sub_entity_checks, field)
            ...
            stmt = select(Sub_entity_checks, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(Sub_entity_checks, field_name):
                        stmt = stmt.order_by(getattr(Sub_entity_checks, field_name).desc())
                else:
                    if hasattr(Sub_entity_checks, sort):
                        stmt = stmt.order_by(getattr(Sub_entity_checks, sort))
            else:
                stmt = stmt.order_by(Sub_entity_checks.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Sub_entity_checks)
                for condition in conditions:
    ...
    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Sub_entity_checks]:
        ...
        try:
            ...
            column = getattr(Sub_entity_checks, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Sub_entity_checks).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
