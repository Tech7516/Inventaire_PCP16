...
class Inventory_itemsService:
    ...
    async def get_list(
        ...
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Inventory_items, field):
                        column = getattr(Inventory_items, field)
            ...
            stmt = select(Inventory_items, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(Inventory_items, field_name):
                        stmt = stmt.order_by(getattr(Inventory_items, field_name).desc())
                else:
                    if hasattr(Inventory_items, sort):
                        stmt = stmt.order_by(getattr(Inventory_items, sort))
            else:
                stmt = stmt.order_by(Inventory_items.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Inventory_items)
                for condition in conditions:
    ...
    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Inventory_items]:
        ...
        try:
            ...
            column = getattr(Inventory_items, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Inventory_items).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
