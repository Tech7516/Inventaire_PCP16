...
# ------------------ Service Layer ------------------
class Lot_configsService:
    """Service layer for Lot_configs operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Lot_configs]:
        """Create a new lot_configs"""
        try:
            obj = Lot_configs(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created lot_configs with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating lot_configs: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Lot_configs]:
        """Get lot_configs by ID"""
        try:
            query = select(Lot_configs).where(Lot_configs.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching lot_configs {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of lot_configss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Lot_configs, field):
                        column = getattr(Lot_configs, field)
            ...
            stmt = select(Lot_configs, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Lot_configs, field_name):
                        stmt = stmt.order_by(getattr(Lot_configs, field_name).desc())
                else:
                    if hasattr(Lot_configs, sort):
                        stmt = stmt.order_by(getattr(Lot_configs, sort))
            else:
                stmt = stmt.order_by(Lot_configs.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Lot_configs)
                for condition in conditions:
            ...
            logger.error(f"Error fetching lot_configs list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Lot_configs]:
        """Update lot_configs"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Lot_configs {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated lot_configs {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating lot_configs {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Lot_configs {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted lot_configs {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting lot_configs {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Lot_configs]:
        """Get lot_configs by any field"""
        try:
            if not hasattr(Lot_configs, field_name):
                raise ValueError(f"Field {field_name} does not exist on Lot_configs")
            column = getattr(Lot_configs, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Lot_configs).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching lot_configs by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Lot_configs]:
        """Get list of lot_configss filtered by field"""
        try:
            if not hasattr(Lot_configs, field_name):
                raise ValueError(f"Field {field_name} does not exist on Lot_configs")
            column = getattr(Lot_configs, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Lot_configs.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching lot_configss by {field_name}: {str(e)}")
            raise