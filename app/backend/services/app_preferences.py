...
# ------------------ Service Layer ------------------
class App_preferencesService:
    """Service layer for App_preferences operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[App_preferences]:
        """Create a new app_preferences"""
        try:
            obj = App_preferences(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created app_preferences with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating app_preferences: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[App_preferences]:
        """Get app_preferences by ID"""
        try:
            query = select(App_preferences).where(App_preferences.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching app_preferences {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of app_preferencess"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(App_preferences, field):
                        column = getattr(App_preferences, field)
            ...
            stmt = select(App_preferences, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(App_preferences, field_name):
                        stmt = stmt.order_by(getattr(App_preferences, field_name).desc())
                else:
                    if hasattr(App_preferences, sort):
                        stmt = stmt.order_by(getattr(App_preferences, sort))
            else:
                stmt = stmt.order_by(App_preferences.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(App_preferences)
                for condition in conditions:
            ...
            logger.error(f"Error fetching app_preferences list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[App_preferences]:
        """Update app_preferences"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"App_preferences {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated app_preferences {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating app_preferences {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete app_preferences"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"App_preferences {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted app_preferences {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting app_preferences {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[App_preferences]:
        """Get app_preferences by any field"""
        try:
            if not hasattr(App_preferences, field_name):
                raise ValueError(f"Field {field_name} does not exist on App_preferences")
            column = getattr(App_preferences, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(App_preferences).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching app_preferences by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[App_preferences]:
        """Get list of app_preferencess filtered by field"""
        try:
            if not hasattr(App_preferences, field_name):
                raise ValueError(f"Field {field_name} does not exist on App_preferences")
            column = getattr(App_preferences, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(App_preferences.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching app_preferencess by {field_name}: {str(e)}")
            raise